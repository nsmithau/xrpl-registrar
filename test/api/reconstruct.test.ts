...
describe("toAccountData", () => {
  it("returns the latest AccountRoot state and null when deleted or absent", () => {
    const entries = [
      entry(10, 0, [modified("AccountRoot", "ACC", { Account: "rA", Balance: "1000", Sequence: 5 })]),
      entry(12, 0, [modified("AccountRoot", "ACC", { Account: "rA", Balance: "1500", Sequence: 6 })]),
    ];
    expect(toAccountData(entries, "rA")?.Balance).toBe("1500");
    expect(toAccountData(entries, "rOther")).toBeNull();

    const withDelete = [...entries, entry(13, 0, [deleted("AccountRoot", "ACC", { Account: "rA" })])];
...
describe("toMptHolders", () => {
  it("reconstructs current holders, latest state wins, deleted excluded, filtered by issuance", () => {
    const entries = [
      entry(100, 0, [created("MPToken", "OBJ_A", { Account: "rA", MPTokenIssuanceID: "MPT", MPTAmount: "100" })]),
      entry(101, 0, [modified("MPToken", "OBJ_A", { Account: "rA", MPTokenIssuanceID: "MPT", MPTAmount: "250" })]),
      entry(100, 0, [created("MPToken", "OBJ_B", { Account: "rB", MPTokenIssuanceID: "MPT", MPTAmount: "50" })]),
      entry(102, 0, [deleted("MPToken", "OBJ_B", { Account: "rB", MPTokenIssuanceID: "MPT", MPTAmount: "0" })]),
      entry(100, 0, [created("MPToken", "OBJ_C", { Account: "rC", MPTokenIssuanceID: "OTHER", MPTAmount: "5" })]),
    ];
  ...
  it("breaks ties within a ledger by TransactionIndex", () => {
    const entries = [
      entry(200, 0, [modified("MPToken", "OBJ_D", { Account: "rD", MPTokenIssuanceID: "MPT", MPTAmount: "10" })]),
      entry(200, 5, [modified("MPToken", "OBJ_D", { Account: "rD", MPTokenIssuanceID: "MPT", MPTAmount: "20" })]),
    ];
...
describe("toAccountLines", () => {
  ...
  it("excludes deleted trustlines", () => {
    const state = { Balance: { currency: "USD", value: "0" }, LowLimit: { issuer: "rA", value: "0" }, HighLimit: { issuer: "rB", value: "0" } };
    const entries = [
