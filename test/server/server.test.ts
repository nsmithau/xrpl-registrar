      ...
      return Promise.resolve({
        result: {
          ...
          info: {
            ...
            validated_ledger: {
              seq: 100, age: 1, hash: "0".repeat(64),
              base_fee_xrp: 0.00001, reserve_base_xrp: 10, reserve_inc_xrp: 2,
            },
          },
        },
        warnings: [],
...
describe("ArchiveServer", () => {
  ...
  beforeAll(async () => {
    ...
    await txns.ingest({ hash: "T1", ledgerIndex: 100, txType: "Payment", txBlob: new Uint8Array([1]), metaBlob: new Uint8Array([2]), provenance: PROV, accounts: ["rInScope"] });
    await txns.ingest({ hash: "T2", ledgerIndex: 200, txType: "Payment", txBlob: new Uint8Array([3]), metaBlob: new Uint8Array([4]), provenance: PROV, accounts: ["rInScope"] });
    await db.query("INSERT INTO coverage (address, from_ledger, to_ledger, reason) VALUES ($1,$2,$3,$4)", ["rInScope", 100, 200, "test"]);
  ...
  it("serves account_tx over HTTP JSON-RPC", async () => {
    const r = await fetch(`http://127.0.0.1:${port}`, {
      ...
      body: JSON.stringify({ method: "account_tx", params: [{ account: "rInScope", api_version: 2, binary: true }] }),
    });
  ...
  it("serves account_tx over WebSocket, echoing id and status", async () => {
    const res = await wsRpc(`ws://127.0.0.1:${port}`, {
      id: 7, command: "account_tx", account: "rInScope", api_version: 2, binary: true,
    });
  ...
  it("returns an error status over WebSocket for out-of-scope", async () => {
    const res = await wsRpc(`ws://127.0.0.1:${port}`, {
      id: 8, command: "account_tx", account: "rStranger", api_version: 2,
    });
  ...
  it("is reachable by an xrpl.js client", async () => {
    ...
    try {
      const res = await client.request({ command: "account_tx", account: "rInScope", binary: true } as Parameters<Client["request"]>[0]);
