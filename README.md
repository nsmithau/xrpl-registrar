...
`gateway_balances` is the IOU issuer-level view (the analogue of `mpt_holders`): given an issuer `account` it returns `obligations` per registered IOU currency in circulation, as of `ledger_index` (default latest), with `hotwallet` accounts broken out into `balances`. It is **fail-closed** — an untracked issuer returns `notInArchive`, and a ledger outside the guaranteed coverage returns an explicit `outOfCoverage` error rather than a possibly-incomplete total. The `assets` field (the issuer's own holdings of other tokens) is out of scope and omitted, flagged by warning `65004`.
...
| `CLIO_HTTP_ENDPOINT`         | no       | _(WebSocket)_ | Optional Clio HTTP JSON-RPC endpoint. When set, the heavy paged `account_tx` backfill/heal uses it — a connection pool parallelises far better than the single WebSocket socket. The tail and forwarding stay on `CLIO_ENDPOINT`. |
