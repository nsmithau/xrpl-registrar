...
| -------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Archive-scoped reads | `account_tx`, `tx`, `account_info`, `account_lines`, `mpt_holders`, `gateway_balances` | Served from the archive, scope-checked. Out-of-scope → `notInArchive`; honest coverage ranges.                                                                              |
...
# Total obligations of an IOU issuer (the IOU analogue of mpt_holders)
curl -s http://127.0.0.1:51234 -H 'content-type: application/json' \
  -d '{"method":"gateway_balances","params":[{"account":"r<issuer>","api_version":2}]}'
...
`gateway_balances` is the IOU issuer-level view (the analogue of `mpt_holders`): given an issuer `account` it returns `obligations` per registered IOU currency in circulation, as of `ledger_index` (default latest), with `hotwallet` accounts broken out into `balances`. Obligations are keyed by the on-wire currency exactly as Clio returns it (a 3-char code as-is, a non-standard code as its 40-hex form), so a Clio/`xrpl.js` client keys identically against us. It is **fail-closed** — an untracked issuer returns `notInArchive`, and a ledger outside the guaranteed coverage returns an explicit `outOfCoverage` error rather than a possibly-incomplete total. The `assets` field (the issuer's own holdings of other tokens) is out of scope and omitted, flagged by warning `65004` ([ADR-017](docs/adr/adr-017-gateway-balances-for-iou-issuers.md)).
...
| `DATABASE_URL`               | no       | —             | Postgres connection URI (e.g. `postgres://user:pw@host:5432/archive`). Selects a networked Postgres server instead of the in-process engine. Setting this **and** `DATABASE_DIR` is an error: they are different archives, so silently choosing one would look like data loss. Run **one process** per URL — the concurrency governor is still in-process (ROADMAP #3). |
| `STORAGE_ENGINE`             | no       | —             | Optional pin: `postgres` makes a missing `DATABASE_URL` a startup error (the container image sets this — an ephemeral filesystem must never fall back to in-process PGlite); `pglite` forbids `DATABASE_URL`.                                                                                                                                                           |
| `DATABASE_POOL_MAX`          | no       | `10`          | Maximum pooled connections. Only used with `DATABASE_URL`. Must be a positive integer.                                                                                                                                                                                                                                                                                  |
| `DATABASE_SSL`               | no       | _(see notes)_ | TLS to the Postgres server. Unset: TLS **on** for a non-loopback host, otherwise leave it to the URL's `sslmode` / `PGSSLMODE`. Set `true`/`false` to force. Only used with `DATABASE_URL`.                                                                                                                                                                             |
...
| `ADMIN_HOST`                 | no       | `127.0.0.1`   | Bind address for the admin API + dashboard. Loopback by default (reach it over SSH). A container or LAN deployment sets a LAN address or `0.0.0.0` — the admin port speaks plain HTTP, so front it with a TLS-terminating proxy ([ADR-019](docs/adr/adr-019-container-image-and-lan-exposure.md)).                                                                      |
| `ADMIN_SECURE_COOKIE`        | no       | `false`       | Mark the dashboard session cookie `Secure`. Set `true` once the dashboard is served over HTTPS by that proxy.                                                                                                                                                                                                                                                           |
